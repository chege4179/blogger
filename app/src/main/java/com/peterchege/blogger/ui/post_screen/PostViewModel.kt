...
class PostViewModel @Inject constructor(
    ...
    private fun getPost(postId:String){
        getPostUseCase(postId = postId).onEach { result ->
            when(result){
                ...
                is Resource.Error -> {
                    _state.value = PostDetailState(error = result.message ?: "This does not exist or has been deleted")
                }
                is Resource.Loading -> {
    ...
    fun onDialogConfirm(scaffoldState: ScaffoldState) {
        ...
        savedStateHandle.get<String>("postId")?.let { postId->
            ...
            Log.d("Comment Logged In",sharedPreferences.getString(Constants.LOGIN_USERNAME,null)!!)
    ...
    private fun addViewCount(){
        viewModelScope.launch {
            ...
            try {
                val response = repository.addView(Viewer(
    ...
    fun followUser(){
        try {
            ...
            val userId = sharedPreferences.getString(Constants.LOGIN_ID,null)
            viewModelScope.launch {
                val followResponse = repository.followUser(FollowUser(
    ...
    fun unfollowUser(){
        try {
            ...
            val userId = sharedPreferences.getString(Constants.LOGIN_ID,null)
            viewModelScope.launch {
                val followResponse = repository.unfollowUser(FollowUser(
                    followerUsername = username!!,
                    followerFullname = fullname!!,
                    followerId = userId!!,
                    followedUsername = _state.value.post!!.postAuthor,
    ...
    private fun postComment(commentBody: CommentBody,scaffoldState: ScaffoldState){
        postCommentUseCase(commentBody).onEach { result ->
            when(result){
                is Resource.Success -> {
                    _commentResponseState.value = CommentResponseState(result.data!!.msg, false,result.data.success)
                ...
                is Resource.Loading -> {
                    _commentResponseState.value = CommentResponseState("posting your comment......", true,false)
